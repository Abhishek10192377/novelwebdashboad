...
const Category = () => {
  ...
  const fetchCategory = async () => {
    try {
      const res = await axios.get('http://localhost:3000/api/read_category');
  ...
  const submitHandler = async (e) => {
    ...
    try {
      await axios.post('http://localhost:3000/api/insert_category', formData, {
  ...
  const updateHandler = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:3000/api/update_category/${editData._id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
  ...
  const deleteHandler = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/api/delete_category/${id}`);
      fetchCategory();
    } catch (err) {
