...
const Novelwrite = () => {
  ...
  const fetchNovels = async () => {
    try {
      const res = await axios.get(`http://localhost:3000/api/novels/read/${id}`);
  ...
  const submitHandler = async (event) => {
    ...
    try {
      if (novelExists) {
        await axios.put(`http://localhost:3000/api/novels/update/${novelId}`, { message });
        toast.success("Novel updated successfully!");
      } else {
        await axios.post("http://localhost:3000/api/novels/insert", { message, Book: id });
  ...
  const deleteHandler = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/api/novels/delete/${id}`);
