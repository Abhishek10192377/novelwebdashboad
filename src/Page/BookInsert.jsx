...
const BookInsert = () => {
  ...
  const fetchBook = async () => {
    try {
      const response = await axios.get("http://localhost:3000/api/books/allRead");
  ...
  const submitHandler = async (event) => {
    ...
    try {
      await axios.post("http://localhost:3000/api/books/insert", formData, {
  ...
  const updateHandler = async (event) => {
    ...
    try {
      await axios.put(`http://localhost:3000/api/books/update/${editData._id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
